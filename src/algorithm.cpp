...
std::stack<Board_ptr> reverseBoardStackOrder(std::stack<Board_ptr> stack) {
...
stringToBoardArrayRepresentation(const std::string &initString) {
  ...
  for (auto &c : initString) {
    initialStateArray.data()[i++] = (uint32_t)(
        c -
        48); //	converting characters in initialState string to numerical.
  }
...
std::string PDS_Visited_List(std::string const initialState,
  ...
  bool success = false;
  std::set<hash_t> visitedList; // The simplest self-banacing
                                // Red-Black tree variety in the STL
  ...
  while (depth_stop != ultimateMaxDepth) {
    ...
    hash_t c = ((S->getHash()) ^
                (((hash_t)S->getFCost())
                 << 20)); //	generate entry for "enhanced" visited list
    ...
    while (!qStack.empty()) {
      ...
      if (testForGoalState(B)) {
        ...
        // print_SUCCESS(*B, maxQLength, numOfStateExpansions,
        // actualRunningTime);
      ...
      while (!(expandedStack.empty())) {
        ...
        if (getPathLength(top) != depth_stop) {
          ...
          if (visitedList.count(c) == false) { // this is the test. As a set is
            ...
            qStack.push(top);
            visitedList.insert(
                c); // add the relevant entry to the visited list.
          }
        }
        expandedStack.pop();
      }
...
std::string bestFirstSearch_Visited_List(std::string const initialState,
  ...
  while (enqueuedTree.empty() == false) {
    maxQLength = std::max(maxQLength,
                          (int)enqueuedTree.size()); //	keeping track of Q size

    B = enqueuedTree.begin()->second;
    enqueuedTree.erase(
        enqueuedTree.begin()); // pop the reference to B off the Q.
...
std::string uniformCost_Exp_List(std::string const initialState,
  ...
  enqueuedTree.insert(
      std::make_pair(S->getFCost(), S)); //	first element in Q...
  enqHashMap.insert(
      std::make_pair(S->getHash(), S)); //	...and to proxy hash map
      ...
      if (strict_expanded_list[top->getHash()] == false) {
        if ((enqHashMap_it = enqHashMap.find(top->getHash())) ==
          ...
          enqueuedTree.insert(
              std::make_pair(top->getFCost(), top)); //	add element in Q...
          enqHashMap.insert(
              std::make_pair(top->getHash(), top)); //	...and to hash map
        } else {
          if (top->getFCost() < enqHashMap_it->second->getFCost()) {
            enqueuedTree_it =
                enqueuedTree.find(enqHashMap_it->second->getFCost());
            ...
            if (enqueuedTree_it != enqueuedTree.end())
              enqueuedTree.erase(enqueuedTree_it); //	remove 'older' and more
                                                   //expensive entry from Q
            enqueuedTree.insert(std::make_pair(
                top->getFCost(), top)); //	add element in Q...
            enqHashMap.insert(std::make_pair(top->getHash(),
                                             top)); //	...and to hash map
          }
        }
      }
...
std::string aStar(std::string const initialState, std::string const goalState,
  ...
  std::map<hash_t, Board_ptr>::iterator
      enqHashMap_it; //	defining a specific
  ...
  enqHashMap.insert(std::make_pair(
      S->getHash(),
      S)); //	...and add accompanying Board state to hash-based proxy map

  while (enqueuedTree.empty() == false) {
    maxQLength = std::max(maxQLength,
                          (int)enqueuedTree.size()); //	keeping track of Q size

    B = enqueuedTree.begin()
            ->second; //	B = Board object held at root node of Q
    enqueuedTree.erase(enqueuedTree.begin()); //	pop root node.
    if ((enqHashMap_it = enqHashMap.find(B->getHash())) != enqHashMap.end()) {
      enqHashMap.erase(enqHashMap_it); //	...the same with the
                                       //accompanying
...
std::string aStar_Exp_List(std::string const initialState,
  ...
  enqueuedTree.insert(
      std::make_pair(S->getFCost(), S)); //	Add S as first element in Q...
  enqHashMap.insert(
      std::make_pair(S->getHash(), S)); //	...and add accompanying
    ...
    while (expandedStack.empty() == false) {
      ...
      if (strict_expanded_list[top->getHash()] == false) {
        ...
        if (enqHashMap_it ==
          ...
          if (top->getFCost() <
              enqHashMap_it->second->getFCost()) { //	if new state is cheaper
                                                   //than old state, replace it.
            enqueuedTree_it =
                enqueuedTree.find(enqHashMap_it->second->getFCost());
            while (enqueuedTree_it->second->getHash() != top->getHash()) {
              ++enqueuedTree_it;
            ...
            enqueuedTree.insert(std::make_pair(
                top->getFCost(),
                top)); //	...before adding the new state to Q...
            enqHashMap.insert(std::make_pair(top->getHash(),
                                             top)); //	...and to proxy hash map
          }
        }
      }
      expandedStack.pop();
