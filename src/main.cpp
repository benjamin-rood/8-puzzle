...
void AnimateSolution(string const initialState, string const goalState,
                     string path) {
  ...
  for (int i = 0; i < path.length(); i++) {
    ...
    case 'U':
      B = std::make_shared<Board>(*B, UP);
      cout << "[UP]" << endl;
      break;
    case 'D':
      B = std::make_shared<Board>(*B, DOWN);
      cout << "[DOWN]" << endl;
      break;
    case 'L':
      B = std::make_shared<Board>(*B, LEFT);
      cout << "[LEFT]" << endl;
      break;
    case 'R':
      B = std::make_shared<Board>(*B, RIGHT);
      cout << "[RIGHT]" << endl;
      break;
    }
    strState = getStateString(B);
...
int main(int argc, const char *argv[]) {
  ...
  if (argc < 5) {
    cout << "SYNTAX: main.exe <TYPE_OF_RUN = batch_run/single_run> "
            "ALGORITHM_NAME \"INITIAL STATE\" \"GOAL STATE\" "
         << endl;
    exit(0);
  }
  ...
  if (algorithmSelected == "PDS_Visited_List") {
    path = PDS_Visited_List(initialState, goalState, numOfStateExpansions,
                            maxQLength, actualRunningTime, 500);

  } else if (algorithmSelected == "bestFirstSearch_Visited_List") {
    path = bestFirstSearch_Visited_List(initialState, goalState,
                                        numOfStateExpansions, maxQLength,
                                        actualRunningTime);

  } else if (algorithmSelected == "uniformCost_Exp_List") {
    path = uniformCost_Exp_List(initialState, goalState, numOfStateExpansions,
                                maxQLength, actualRunningTime);

  } else if (algorithmSelected == "aStar_Misplaced") {
    path = aStar(initialState, goalState, numOfStateExpansions, maxQLength,
                 actualRunningTime, misplacedTiles);

  } else if (algorithmSelected == "aStar_Manhattan") {
    path = aStar(initialState, goalState, numOfStateExpansions, maxQLength,
                 actualRunningTime, manhattanDistance);

  } else if (algorithmSelected == "aStar_Exp_List_Misplaced") {
    path = aStar_Exp_List(initialState, goalState, numOfStateExpansions,
                          maxQLength, actualRunningTime, misplacedTiles);

  } else if (algorithmSelected == "aStar_Exp_List_Manhattan") {
    path = aStar_Exp_List(initialState, goalState, numOfStateExpansions,
                          maxQLength, actualRunningTime, manhattanDistance);
  }
  ...
  if (typeOfRun == "batch_run") {
    ...
    cout << setprecision(6) << std::setfill(' ') << std::fixed << std::right
         << ' ' << setw(10) << pathLength;
    cout << setprecision(6) << std::setfill(' ') << std::fixed << std::right
         << ' ' << setw(10) << numOfStateExpansions;
    cout << setprecision(6) << std::setfill(' ') << std::fixed << std::right
         << ' ' << setw(15) << maxQLength;
    cout << setprecision(6) << std::setfill(' ') << std::fixed << std::right
         << ' ' << setw(15) << actualRunningTime << endl;

  } else if (typeOfRun == "single_run") {
    cout << setprecision(6) << setw(25) << std::setfill(' ') << std::right
         << endl
         << endl
         << "Initial State:" << std::fixed << ' ' << setw(12) << initialState
         << endl;
    cout << setprecision(6) << setw(25) << std::setfill(' ') << std::right
         << "Goal State:" << std::fixed << ' ' << setw(12) << goalState << endl;
    cout << setprecision(6) << setw(25) << std::setfill(' ') << std::right
         << endl
         << "Path Length:" << std::fixed << ' ' << setw(12) << pathLength
         << endl;
    cout << setprecision(6) << setw(25) << std::setfill(' ') << std::right
         << "Num Of State Expansions:" << std::fixed << ' ' << setw(12)
         << numOfStateExpansions << endl;
    cout << setprecision(6) << setw(25) << std::setfill(' ') << std::right
         << "Max Q Length:" << std::fixed << ' ' << setw(12) << maxQLength
         << endl;
    cout << setprecision(6) << setw(25) << std::setfill(' ') << std::right
         << "Actual Running Time:" << std::fixed << ' ' << setprecision(6)
         << setw(12) << actualRunningTime << endl;
    cout << "=================================================================="
            "=============================================="
         << endl
         << endl;
