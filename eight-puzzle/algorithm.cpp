...
std::string uniformCost_Exp_List(std::string const initialState,
  ...
  enqHashMap.insert(std::make_pair(
      S->getHash(), S)); //	...and add accompanying Board state to hash map
      ...
      if (strict_expanded_list[top->getHash()] == false) {
        if ((enqHashMap_it = enqHashMap.find(top->getHash())) ==
          ...
          if (top->getFCost() < enqHashMap_it->second->getFCost()) {
            ...
            if (enqueuedTree_it == enqueuedTree.end())
              enqueuedTree.erase(enqueuedTree_it);
...
std::string aStar(std::string const initialState, std::string const goalState,
  ...
  std::multimap<fCost_t, Board_ptr> enqueuedTree; //	where Q
                                                                 //is a
                                                                 //Red-Black
                                                                 //tree which
                                                                 //supports more
  ...
  //	In this case, our fCost = pathlength + admissible heuristic.

  std::map<hash_t, Board_ptr> enqHashMap;	//	using a
														//	hash-based map as a
														//	proxy for faster search
														//	through Q.
	
  std::map<hash_t, Board_ptr>::iterator enqHashMap_it; //	defining
                                                                   //	a specific
                                                                   //	iterator to
                                                                   //	access the
                                                                   //	hash-based
			...
			if (strict_expanded_list[top->getHash()] == false)	{
				...
				if (enqHashMap_it == enqHashMap.end())	{	//	Not already in queue, just directly insert it
					...
					if (top->getFCost() < enqHashMap_it->second->getFCost()) {	//	if new state is cheaper than old state, replace it.
						...
						enqHashMap.insert(std::make_pair(top->getHash(), top));		//	...and add accompanying entry to hash map
					}
				}
			}
			expandedStack.pop();
