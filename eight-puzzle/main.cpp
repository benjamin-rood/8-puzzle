...
int main(int argc, const char * argv[])
{
	...
	std::multimap<fCost_t, Board> q_EnqueuedTree;
	std::multimap<fCost_t, Board>::iterator q_EnqueuedTree_It;
	std::map<hash_t, Board> q_hashMap;				//	using a map so we can arrange by hash for faster comparisons!
	std::map<hash_t, Board>::iterator q_hashMap_It;
...
//	generateManhattanHeuristic(*S);
	...
	while ( q_EnqueuedTree.empty() == false )
	{
		...
		B = std::make_shared<Board>(q_EnqueuedTree.begin()->second);
...
//			generateManhattanHeuristic(*top);
			...
			if ( strict_expanded_list[top->getHash()] == false ) {
				if ( (q_hashMap_It = q_hashMap.find( top->getHash() )) == q_hashMap.end() )	{	//	Not already in queue, just directly insert it
					...
					if ( top->getFCost() < q_hashMap_It->second.getFCost() ) {
						...
						while ( q_EnqueuedTree_It->second.getHash() != top->getHash() ) ++q_EnqueuedTree_It;
	...
	// BREADTH-FIRST SEARCH:
	{
		...
		std::shared_ptr<Board> start = std::make_shared<Board>(  simpleState3 );
